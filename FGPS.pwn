...
removefromname(string[])
{
	...
	format(nameholder, sizeof nameholder, string);
	while ((i_pos = strfind(nameholder, "_", false, i_pos)) != -1) nameholder[i_pos] = ' ';
	return nameholder;
}

//Change this to a lower number if you want it to look smoother
//The lower the timer the bigger the memory needed.
...
CMD:gpssave(playerid, params[])
{
	...
	format(query, sizeof(query), "INSERT INTO `FGPSSystem` (`LocationX`,`LocationY`,`LocationZ`, `Name`) VALUES('%f','%f','%f','%s');",PPos[0], PPos[1], PPos[2], removefromname(params));
	...
	db_query(GPSDB,query);
	ReloadDatabase();
	format(string,sizeof(string),"You have added the place: {00FF33}%s {FFFFFF}to the database", removefromname(params));
	SendClientMessage(playerid, 0xFFFFFFFF, string);
	return 1;
}
...
CMD:gpsrename(playerid, params[])
{
    if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, 0xE60000FF, "You are not an Admin!");
	new string[128], query[256], tmp[2][128];
	if(isnull(params)) return SendClientMessage(playerid, 0xFFFFFF, "/gpsedit [Old Name] [New Name]");
	split(params, tmp, ' ');
	format(query, sizeof(query), "UPDATE `FGPSSystem` SET `Name` = '%s' WHERE `Name` = '%s'", removefromname(tmp[1]), removefromname(tmp[0]));
	db_query(GPSDB,query);
	ReloadDatabase();
	format(string,sizeof(string),"You have edit the name of the place: {00FF33}%s {FFFFFF}to {00FF33}%s", removefromname(tmp[0]), removefromname(tmp[1]));
	SendClientMessage(playerid, 0xFFFFFFFF, string);
	return 1;
}
...
public OnPlayerEnterCheckpoint(playerid)
{
	...
	if (IsPlayerConnected(playerid))
	{
		if(IsPlayerInCheckpoint(playerid))
		{
			...
			if (GetPVarInt(playerid,"YEAH") == 1)
			{
				new string[64], name[144];
				GetPVarString(playerid, "FAIL", name, sizeof name);
				format(string, sizeof string, "You have reached your destination {FF0000}%s", name);
			   	SendClientMessage(playerid, 0xFFFFFFFF, string);
				DestroyObject(GPSObject[playerid]);
				SetPVarInt(playerid,"YEAH",0);
				DeletePVar(playerid,"Spongebob");
...
task QuarterOfASecondTimer[250]()
{
	foreach(new playerid : Player)
	{
		if(GetPVarInt(playerid,"YEAH") == 1)
		{
			new Float:VPos[3], Float:Rotation, TDString[144];
